...
/**
 * We need support for the 'PATCH' method and there is no way to implement this
 * into the {@link LocalTestServer} of apache. Preferably we would only override
 * one method, but this is not possible.Therefore the very ugly copy & paste.
 * The alternative is to depend on a patch tag, but this is more maintainable.
 * 
 * See https://issues.apache.org/jira/browse/HTTPCLIENT-1239
 * 
 */
public class LocalTestServerHack {
   ...
   * 
   * @param proc
   *          the HTTP processors to be used by the server, or <code>null</code>
   *          to use a {@link #newProcessor default} processor
   * @param reuseStrat
   ...
   *          transport security
   */
  public LocalTestServerHack(final BasicHttpProcessor proc, final ConnectionReuseStrategy reuseStrat,
      final HttpResponseFactory responseFactory, final HttpExpectationVerifier expectationVerifier, final HttpParams params,
    ...
    this.httpservice = new HttpService(proc != null ? proc : newProcessor(),
        reuseStrat != null ? reuseStrat : newConnectionReuseStrategy(), responseFactory != null ? responseFactory
            : newHttpResponseFactory(), handlerRegistry, expectationVerifier, params != null ? params : newDefaultParams());
   ...
   *          SSL context
   */
  public LocalTestServerHack(final SSLContext sslcontext) {
   ...
   *          use {@link #newDefaultParams default} parameters
   */
  public LocalTestServerHack(BasicHttpProcessor proc, HttpParams params) {
  ...
  protected HttpProcessor newProcessor() {
    return new ImmutableHttpProcessor(new HttpResponseInterceptor[] { new ResponseDate(), new ResponseServer(), new ResponseContent(),
  ...
  protected HttpParams newDefaultParams() {
    ...
    params.setIntParameter(CoreConnectionPNames.SO_TIMEOUT, 60000).setIntParameter(CoreConnectionPNames.SOCKET_BUFFER_SIZE, 8 * 1024)
        .setBooleanParameter(CoreConnectionPNames.STALE_CONNECTION_CHECK, false)
        .setBooleanParameter(CoreConnectionPNames.TCP_NODELAY, true).setParameter(CoreProtocolPNames.ORIGIN_SERVER, "LocalTestServer/1.1");
    return params;
  }
  ...
  /**
   * Creates an instance of {@link DefaultHttpServerConnection} to be used in
   * the Worker thread.
   * <p>
   * This method can be overridden in a super class in order to provide a
   * different implementation of the {@link DefaultHttpServerConnection}.
   * 
